...
class Mingguan extends CI_Controller {
	...
	function ambiladjustment()
	{
		...
		$periode_awal = $pecah[1];
		$periode_akhir = $pecah[2];		
		$data = $this->db->query("select * from t_adjustment_mingguan where noinduk='$noinduk' and periode_awal='$periode_awal' and periode_akhir='$periode_akhir'")->result();
	...
	function ambil_karyawan_seksi()
	{
		...
		$result = $this->db->query("select * from t_karyawan where seksi='$seksi' and upah='Y' and tkaryawan='H' and saktif='1'")->result_array();
	...
	function ambil_karyawan_seksi_insentif()
	{
		...
		$result = $this->db->query("select * from t_karyawan where seksi='$seksi' and upah='Y' and tkaryawan='H' and saktif='1' and skerja='T'")->result_array();
	...
	function simpan_insentif()
	{
		$noinduk = $this->input->post('noinduk');
		$insentif = $this->input->post('insentif');
		$sqlseksi = $this->db->query("select * from t_karyawan where noinduk='$noinduk' and tkaryawan = 'H' and saktif='1'")->result_array();
		if($sqlseksi)
		{
	...
	function simpan_spl()
	{
		...
		if($cek > 0)
		{
			$act = $this->db->query("update t_spl_mingguan set status = '$status',
				...
				$act = $this->db->query("insert into t_spl_mingguan(noinduk,periode_awal,periode_akhir,tgl1,tgl2,tgl3,tgl4,tgl5,tgl6,tgl7,tgl8,tgl9,tgl10,tgl11,tgl12,tgl13,tgl14,tgl15,tgl16,tgl17,tgl18,tgl19,tgl20,tgl21,tgl22,tgl23,tgl24,tgl25,tgl26,tgl27,tgl28,tgl29,tgl30,tgl31,status)values('$noinduk','$periode_awal','$periode_akhir','$tgl1','$tgl2','$tgl3','$tgl4','$tgl5','$tgl6','$tgl7','$tgl8','$tgl9','$tgl10','$tgl11','$tgl12','$tgl13','$tgl14','$tgl15','$tgl16','$tgl17','$tgl18','$tgl19','$tgl20','$tgl21','$tgl22','$tgl23','$tgl24','$tgl25','$tgl26','$tgl27','$tgl28','$tgl29','$tgl30','$tgl31','$status')");
		}
	...
	function simpan_adjustment()
	{
		...
		$cek = $this->db->query("select * from t_adjustment_mingguan where noinduk='$noinduk' and periode_awal='$periode_awal' and periode_akhir='$periode_akhir'")->num_rows();
		if($cek > 0)
		{
			$act = $this->db->query("update t_adjustment_mingguan set adjustment='$adjustment'
																    where noinduk='$noinduk' and periode_awal='$periode_awal' and periode_akhir='$periode_akhir'");	
		}else{
			$act = $this->db->query("insert into t_adjustment_mingguan(noinduk,periode_awal,periode_akhir,adjustment)values('$noinduk','$periode_awal','$periode_akhir','$adjustment')");
		}
		echo json_encode($act);
	...
	function proses_gaji(){
		...
		$karyawan = $this->db->query("select * from t_karyawan where tkaryawan='H' and saktif='1' and upah='Y'")->result_array();
		foreach($karyawan as $d)
		{
		 ...
		 $this->db->query("delete from t_gaji_mingguan where noinduk='$noinduk' and periode_awal_upah ='$periode_awal_upah' and periode_akhir_upah='$periode_akhir_upah'");
			...
			//total gaji diterima
			$thp =($upah + $premi + $lt + $spl + $juminsentif + (($t3e/30)*14) + (($t3m/30)*14)) - ($jht + $pensiun + $bpjs_kes + $pot_t3m + $pot_absen + $koperasi + $spsi1);
	...
	function proses_slip()
	{
		...
		$sql = "select a.*,b.noslip,b.seksi,b.nama from t_gaji_mingguan a
				join t_karyawan b on b.noinduk=a.noinduk where a.periode_awal_upah='$periode_awal' and a.periode_akhir_upah ='$periode_akhir' and b.tkaryawan ='H' and skerja='$status'";
	...
	function proses_lap_upah_mingguan()
	{
		...
		$sql = "select distinct seksi from t_karyawan where tkaryawan='H' and skerja='$status' and saktif='1' and upah='Y' and pabrik='$pabrik'";
		...
		foreach($dataseksi as $d)
		{
			...
			$spsi1 = '';
			$lain_lain1 = '';
			$sql = "select * from t_karyawan where seksi = '$seksi' and tkaryawan ='H' and skerja='$status' and saktif='1' and upah='Y' and pabrik='$pabrik'";
			...
			foreach($karyawan as $b)
			{
				...
				$sql = "select ((gp + tjab + tmakan + t3m + t3e)/30) * 14 as upah,premi,lt,pot_absen,(jht + pensiun) as bpjs_tk,bpjs_kes,pot_t3m,spsi,koperasi from t_gaji_mingguan where noinduk='$noinduk' and periode_awal_upah='$periode_awal' and periode_akhir_upah='$periode_akhir'";
			...
			$insert = $this->db->query("insert into t_rekap_upah_mingguan(periode_awal,periode_akhir,seksi,upah,lt,premi,bpjs_tk,bpjs_kes,koperasi,spsi,lain_lain,status)values('$periode_awal','$periode_akhir','$seksi','$upah1','$lt1','$premi1','$bpjs_tk1','$bpjs_kes1','$koperasi1','$spsi1','$lain_lain','$status')");
...
//ambil seksi dari tabel karyawan
		$sql = "select * from t_karyawan where tkaryawan='H' and skerja='$status' and saktif='1' and upah='Y' and pabrik='$pabrik'";
		$karyawan = $this->db->query($sql)->result_array();
		...
		foreach($karyawan as $d)
		{
			...
			foreach($gaji as $b)
			{
				$upah = (($b['gp'] + $b['tjab'] + $b['tmakan'] + $b['t3m'])/30)*14;
	...
	function proses_lap_lembur_mingguan()
	{
		...
		//	$lembur1 = '';
		//	$jam	= '';
			$sqlkary = "select * from t_karyawan where seksi = '$seksi' and tkaryawan ='H' and skerja='$status' and saktif='1' and upah='Y' and pabrik='$pabrik'";
	...
	function proses_lap_spl_mingguan()
	{
		...
		{	
			...
			$sql1 = "select * from t_spl_mingguan where noinduk='$noinduk' and periode_awal='$periode_awal' and periode_akhir='$periode_akhir'   and status='$susulan'";
	...
	function ambil_rekap_spl_mingguan()
	{
		...
		$hasil = $this->db->query("select * from t_rekap_spl_mingguan where jam != 0 and status='$status' and susulan='$susulan' and seksi='$seksi' and pawal='$periode_awal' and pakhir ='$periode_akhir'")->result_array();
		$data = array (
			'data' 				=> $hasil,
			'periode_awal'	 	=> $periode_awal,
			'periode_akhir' 	=> $periode_akhir,
