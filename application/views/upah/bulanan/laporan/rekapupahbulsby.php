		...
		<div class="panel panel-success">
			<div class="panel-heading">
				<h2 class="panel-title">
					Laporan Rekap Upah Bulanan Tetap (Untuk Surabaya)
				</h2>
			</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('#proses').click(function() {
				...
				// 	$.ajax({
				// 		type: 'POST',
				// 		url: '<?php echo base_url() . "index.php/bulanan/proses_lap_upah_bulanan_sby" ?>',
				// 		data: {
		...
		function loadData() {
			$.ajax({
				type: 'post',
				url: '<?php echo base_url() . 'index.php/bulanan/ambil_rekap_upah_bulanan_sby' ?>',
				data: {
					bulan: $('[name=bulan]').val(),
					tahun: $('[name=tahun]').val(),
					status: $('[name=status]').val(),
