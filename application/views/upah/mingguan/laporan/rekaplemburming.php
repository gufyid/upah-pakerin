		...
		<div class="panel panel-success">
			<div class="panel-heading">
				<h2 class="panel-title">
					Laporan Rekap Upah Lembur Bulanan Per Seksi
				</h2>
			</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('#proses').click(function() {
				...
				// 	$.ajax({
				// 		type: 'POST',
				// 		url: '<?php echo base_url() . "index.php/mingguan/proses_lap_lembur_mingguan" ?>',
				// 		data: {
		...
		function loadData() {
			$.ajax({
				type: 'post',
				url: '<?php echo base_url() . 'index.php/mingguan/ambil_rekap_lembur_mingguan' ?>',
				data: {
					periode_awal: $('[name=periode_awal]').val(),
					periode_akhir: $('[name=periode_akhir]').val(),
					status: $('[name=status]').val(),
