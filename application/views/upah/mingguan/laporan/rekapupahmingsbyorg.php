...
<body class="no-skin">
	<center>
		<font color="blue"><span id="pesan"></span></font>
	</center>
	<div role="main" class="non-printable">
		<div class="panel panel-success">
			<div class="panel-heading">
				<h2 class="panel-title">
					Laporan Rekap Upah Karyawan Harian / Orang (Untuk Surabaya)
				</h2>
			</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('#proses').click(function() {
				...
				// 	$.ajax({
				// 		type: 'POST',
				// 		url: '<?php echo base_url() . "index.php/mingguan/proses_lap_upah_mingguan_sby_org" ?>',
				// 		data: {
		...
		function loadData() {
			$.ajax({
				type: 'post',
				url: '<?php echo base_url() . 'index.php/mingguan/ambil_rekap_upah_mingguan_sby_org' ?>',
				data: {
					periode_awal: $('[name=periode_awal]').val(),
					periode_akhir: $('[name=periode_akhir]').val(),
					status: $('[name=status]').val(),
